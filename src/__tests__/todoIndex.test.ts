...
describe("TodoIndex.rebuild", () => {
  it("indexes todos from all markdown files", async () => {
  ...
  it("skips files in archive folders", async () => {
    const app = buildMockApp({
  ...
  it("scans all vault files (not limited to specific folders)", async () => {
    const app = buildMockApp({
  ...
  it("notifies listeners after rebuild", async () => {
  ...
  it("skips files with excludeTodos frontmatter", async () => {
    const app = buildMockApp({
      "notes/excluded.md": "---\nexcludeTodos: true\n---\n- [ ] Hidden todo",
  ...
  it("resets nextId on each rebuild", async () => {
    const app = buildMockApp({ "notes/a.md": "- [ ] task" });
    const idx = new TodoIndex(app, baseSettings);
    await idx.rebuild();
...
describe("TodoIndex.updateFile", () => {
  it("adds todos for a new file", async () => {
    const app = buildMockApp({});
    const idx = new TodoIndex(app, baseSettings);
    ...
    await idx.updateFile(file, "- [ ] New todo", cache as any);

    expect(idx.getAllTodos()).toHaveLength(1);
    expect(idx.getAllTodos()[0].text).toBe("New todo");
  });

  it("replaces todos for an existing file", async () => {
    ...
    await idx.updateFile(file, newContent, cache as any);
  ...
  it("removes file from index when file moves to archive", async () => {
    ...
    await idx.updateFile(file, "- [ ] A todo", undefined);
  ...
  it("notifies listeners on update", async () => {
    ...
    await idx.updateFile(file, "- [ ] Task", {
      listItems: [{ task: " ", position: { start: { line: 0 } } }],
    } as any);
...
describe("TodoIndex.removeFile", () => {
  it("removes todos for a deleted file", async () => {
    const app = buildMockApp({ "notes/a.md": "- [ ] Task" });
    const idx = new TodoIndex(app, baseSettings);
    await idx.rebuild();
    expect(idx.getAllTodos()).toHaveLength(1);
  ...
  it("notifies listeners when a file is removed", async () => {
    const app = buildMockApp({ "notes/a.md": "- [ ] Task" });
    const idx = new TodoIndex(app, baseSettings);
    await idx.rebuild();
...
describe("TodoIndex – file rename handling", () => {
  it("preserves todos when a file is renamed", async () => {
    const app = buildMockApp({
    ...
    await idx.updateFile(newFile, content, cache as any);
  ...
  it("updates file reference to new path after rename", async () => {
    const app = buildMockApp({
      "notes/old.md": "- [ ] Task",
    });
    const idx = new TodoIndex(app, baseSettings);
  ...
  it("updates group name when file is moved to a different folder", async () => {
    const app = buildMockApp({
  ...
  it("removes todos when file is moved into archive", async () => {
    const app = buildMockApp({
    ...
    await idx.updateFile(archivedFile, content, cache as any);

    expect(idx.getAllTodos()).toHaveLength(0);
  });

  it("picks up todos when file is moved out of archive", async () => {
    const app = buildMockApp({});
    const idx = new TodoIndex(app, baseSettings);
    await idx.rebuild();
    expect(idx.getAllTodos()).toHaveLength(0);
  ...
  it("updates fileDate when renamed to a dated filename", async () => {
    const app = buildMockApp({
  ...
  it("does not leave stale entries at the old path", async () => {
    const app = buildMockApp({
      "notes/a.md": "- [ ] Task A",
      "notes/b.md": "- [ ] Task B",
    });
  ...
  it("notifies listeners for both remove and re-index", async () => {
    const app = buildMockApp({
      "notes/old.md": "- [ ] Task",
    });
    const idx = new TodoIndex(app, baseSettings);
...
describe("TodoIndex.getAllTodos", () => {
  it("returns todos sorted by fileDate descending then id ascending", async () => {
    const app = buildMockApp({
  ...
  it("files with dates sort before files without dates", async () => {
    const app = buildMockApp({
...
describe("TodoIndex.getTodosFor", () => {
  it("returns todos assigned to a specific person", async () => {
    const app = buildMockApp({
  ...
  it("returns empty array when nobody matches", async () => {
...
describe("TodoIndex.getMyTodos", () => {
  it("returns only unassigned todos", async () => {
    const app = buildMockApp({
      "notes/a.md": "- [ ] My task\n- [ ] @Bob assigned task",
    });
    const idx = new TodoIndex(app, baseSettings);
...
describe("TodoIndex.getTodosInFolder", () => {
  it("returns todos from files within the given folder", async () => {
    const app = buildMockApp({
  ...
  it("returns todos for nested subdirectories", async () => {
    const app = buildMockApp({
...
describe("TodoIndex.getGroupNames", () => {
  it("returns unique group names", async () => {
    const app = buildMockApp({
  ...
  it("orders by fileDate descending", async () => {
    const app = buildMockApp({
      "2026-01-01.md": "- [ ] Jan",
      "2026-03-01.md": "- [ ] Mar",
    });
...
describe("TodoIndex.onChange", () => {
  it("calls listener on rebuild", async () => {
    const app = buildMockApp({ "notes/a.md": "- [ ] task" });
    const idx = new TodoIndex(app, baseSettings);
    const cb = jest.fn();
    idx.onChange(cb);
...
describe("TodoIndex – todo field extraction", () => {
  it("assigns 'Me' for unassigned todos", async () => {
  ...
  it("extracts @mention assignment", async () => {
  ...
  it("extracts fileDate from dated filename", async () => {
  ...
  it("fileDate is null for undated filename", async () => {
  ...
  it("records correct lineNumber", async () => {
    const app = buildMockApp({
      "notes/a.md": "# Heading\n\nSome text\n- [ ] Line 3 task",
    });
    const idx = new TodoIndex(app, baseSettings);
  ...
  it("parses due date from todo text using file date as reference", async () => {
  ...
  it("dueDate is null when no due date in text", async () => {
  ...
  it("builds group name as 'parent / stem'", async () => {
  ...
  it("group name uses grandparent when parent matches stem", async () => {
...
describe("TodoIndex.refreshAliases", () => {
  it("re-resolves assignedToAlias for all existing todos", async () => {
  ...
  it("notifies listeners after refreshing aliases", async () => {
...
describe("TodoIndex.updateFile – team README handling", () => {
  it("triggers refreshAliases when a team README.md is updated", async () => {
    const app = buildMockApp({
    ...
    await idx.updateFile(teamReadme, "---\naliases: []\n---\n", undefined);

    // Should have been notified (refreshAliases calls notify)
    expect(cb).toHaveBeenCalled();
  });

  it("does not trigger refreshAliases for non-team README.md", async () => {
    const app = buildMockApp({
      "notes/a.md": "- [ ] task",
    });
    const idx = new TodoIndex(app, baseSettings);
    ...
    await idx.updateFile(rootReadme, "content", undefined);
